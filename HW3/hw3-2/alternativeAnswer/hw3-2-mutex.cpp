...
mutex checkLock;
mutex workingLock;
vector<bool> working(MAX_VERTICES, false);
...
void maximum_independent_set(int v)
{
    ...
    while (!converged)
    {
        ...
        if (vertex_checked[v])
        {
            lock_guard<mutex> lock(checkLock);
        ...
        else
        {
            ...
            {
                {
                    lock_guard<mutex> lock(workingLock);
                    canGetIn = true;
                ...
                {
                    semaphores.release();
                    this_thread::sleep_for(chrono::nanoseconds(dist(rng)));
                    semaphores.acquire();
                }
            }
